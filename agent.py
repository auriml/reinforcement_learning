...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        a = 0.004
        self.epsilon =   math.cos(self.no_trials*a)

        self.alpha =  math.cos(self.no_trials*a)
    ...
    def get_maxQ(self, state):
        ...
        import operator
        maxQ =  max(self.Q.get(state).iteritems(), key= operator.itemgetter(1))[1]
    ...
    def createQ(self, state):
        ...
        if self.learning is True:
            ...
            if self.Q.has_key(state) is False:
                self.Q[state] = dict()
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning is True:
            #Version with gamma - commented
            #gamma = 1.0
            #learned_value = reward + gamma * self.get_maxQ(state)
            #self.Q[state][action]= (1-self.alpha)*self.Q[state][action] + self.alpha * (learned_value )

            #Version without gamma as this exercise requires
...
def run():
    ...
    #sim = Simulator(env, display=True, update_delay=1, log_metrics=False, optimized= False)
